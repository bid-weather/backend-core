...
WHERE subcategory_id IS NULL AND (
    title LIKE '%폭염%' OR title LIKE '%냉방%' OR title LIKE '%무더위%' OR
    title LIKE '%쿨링%' OR title LIKE '%그늘막%' OR title LIKE '%열사병%' OR
    title LIKE '%온열%' OR title LIKE '%냉풍%' OR title LIKE '%피서%'
    );

UPDATE announcement SET subcategory_id = 4  -- 가뭄 및 용수 확보
WHERE subcategory_id IS NULL AND (
    title LIKE '%가뭄%' OR title LIKE '%용수%' OR title LIKE '%저수지%' OR
    title LIKE '%취수%' OR title LIKE '%급수%' OR title LIKE '%물부족%' OR
    title LIKE '%관개%' OR title LIKE '%수리시설%' OR title LIKE '%양수%'
    );
...
WHERE subcategory_id IS NULL AND (
    title LIKE '%동파%' OR title LIKE '%혹한%' OR title LIKE '%동결%' OR
    title LIKE '%보온%' OR title LIKE '%동절기%' OR title LIKE '%한파%' OR
    title LIKE '%동해%' OR title LIKE '%방한%' OR title LIKE '%보냉%'
    );

UPDATE announcement SET subcategory_id = 7  -- 지반 및 사면 보강
WHERE subcategory_id IS NULL AND (
    title LIKE '%사면%' OR title LIKE '%지반%' OR title LIKE '%옹벽%' OR
    title LIKE '%절개지%' OR title LIKE '%낙석%' OR title LIKE '%토사%' OR
    title LIKE '%산사태%' OR title LIKE '%급경사%' OR title LIKE '%보강공사%' OR
    title LIKE '%석축%' OR title LIKE '%토석류%'
    );

UPDATE announcement SET subcategory_id = 8  -- 수목 관리 및 산불 예방
WHERE subcategory_id IS NULL AND (
    title LIKE '%산불%' OR title LIKE '%수목%' OR title LIKE '%조림%' OR
    title LIKE '%벌목%' OR title LIKE '%나무%' OR title LIKE '%방화선%' OR
...
WHERE subcategory_id IS NULL AND (
    title LIKE '%기상%' OR title LIKE '%재해영향%' OR title LIKE '%용역%' OR
    title LIKE '%설계%' OR title LIKE '%연구%' OR title LIKE '%조사%' OR
    title LIKE '%분석%' OR title LIKE '%계획수립%' OR title LIKE '%타당성%' OR
    title LIKE '%모니터링%'
    );

UPDATE announcement SET subcategory_id = 10  -- 재난 경보 및 통신
WHERE subcategory_id IS NULL AND (
    title LIKE '%경보%' OR title LIKE '%재난문자%' OR title LIKE '%사이렌%' OR
    title LIKE '%CCTV%' OR title LIKE '%계측%' OR title LIKE '%센서%' OR
    title LIKE '%통신망%' OR title LIKE '%경보시스템%' OR title LIKE '%관제%' OR
    title LIKE '%방재%'
    );

UPDATE announcement SET subcategory_id = 11  -- 해양 및 어업 재해
WHERE subcategory_id IS NULL AND (
    title LIKE '%해양%' OR title LIKE '%어업%' OR title LIKE '%항만%' OR
    title LIKE '%방파제%' OR title LIKE '%어항%' OR title LIKE '%태풍%' OR
    title LIKE '%해일%' OR title LIKE '%조류%' OR title LIKE '%해안%' OR
    title LIKE '%어장%'
    );

UPDATE announcement SET subcategory_id = 12  -- 미세먼지 및 황사
WHERE subcategory_id IS NULL AND (
    title LIKE '%미세먼지%' OR title LIKE '%황사%' OR title LIKE '%공기질%' OR
    title LIKE '%대기질%' OR title LIKE '%먼지%' OR title LIKE '%집진%' OR
    title LIKE '%대기오염%' OR title LIKE '%공기정화%' OR title LIKE '%청정%'
    );

UPDATE announcement SET subcategory_id = 13  -- 감염병 방역 및 보건
WHERE subcategory_id IS NULL AND (
    title LIKE '%방역%' OR title LIKE '%감염병%' OR title LIKE '%소독%' OR
    title LIKE '%방생%' OR title LIKE '%보건%' OR title LIKE '%검역%' OR
    title LIKE '%방충%' OR title LIKE '%해충%' OR title LIKE '%위생%' OR
    title LIKE '%역병%'
    );
...
WHERE subcategory_id IS NULL AND (
    ...
    title LIKE '%병충해%' OR title LIKE '%가축%' OR title LIKE '%영농%' OR
    title LIKE '%농업%' OR title LIKE '%작물%' OR title LIKE '%살충%' OR
    title LIKE '%구제역%' OR title LIKE '%조류독감%'
    );
...
WHERE subcategory_id IS NULL AND (
    ...
    title LIKE '%정화조%' OR title LIKE '%하수관%' OR title LIKE '%오염수%' OR
    title LIKE '%수도관%'
    );
